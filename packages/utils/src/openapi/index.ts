...
/**
 * Walk an OpenAPI spec and mark all empty `additionalProperties: {}` with
 * `x-stainless-any: true` so Stainless treats them as `unknown` instead of
 * flagging Schema/IsAmbiguous.
 */
export function markAmbiguousAdditionalProperties(obj: unknown): void {
	if (!obj || typeof obj !== "object") return
	if (Array.isArray(obj)) {
		for (const item of obj) markAmbiguousAdditionalProperties(item)
		return
	}
	const record = obj as Record<string, unknown>
	if (
		...
		Object.keys(record.additionalProperties as object).length === 0
	) {
		record.additionalProperties = { "x-stainless-any": true }
	}
	for (const value of Object.values(record)) {
		markAmbiguousAdditionalProperties(value)
	}
}
